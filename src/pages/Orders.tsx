    ...
    queryFn: async () => {
      try {
        ...
        const orders = await Promise.all(snapshot.docs.map(async (orderDoc) => {
          ...
          const items: OrderItem[] = await Promise.all(itemsSnapshot.docs.map(async (itemDoc) => {
            ...
            if (itemData.product_id) {
              ...
              if (productDoc.exists()) {
                productData = productDoc.data() as Record<string, unknown>;
              }
            }

            return {
              id: String(itemData.product_id || 'unknown'),
              name: String(productData.name || 'Unknown Product'),
              image: parseImageUrl(productData.image_url || 'https://picsum.photos/seed/placeholder/100/100'),
              price: Number(itemData.price_at_time || 0),
          ...
          else if (dbStatus === "cancelled") status = "processing"; // fallback

          return {
            id: orderDoc.id,
            orderNumber: `ORD-${orderDoc.id.substring(0, 8).toUpperCase()}`,
            date: new Date(orderData.created_at as string).toLocaleDateString(),
            status,
            estimatedDelivery: new Date(new Date(orderData.created_at as string).getTime() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString(),
            items,
