...
export default function AdminCustomersPage() {
  ...
  const handleBlockCustomer = async (customer: Customer) => {
    if (!window.confirm(`Are you sure you want to block ${customer.name}?`)) return;
    setBlocking(customer.id);
    try {
      await updateDoc(doc(db, "users", customer.id), { status: "Blocked" });
                    ...
                    <td className="p-3.5 pr-5">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem className="gap-2 text-destructive focus:text-destructive" onClick={() => handleBlockCustomer(customer)} disabled={blocking === customer.id}>
                            {blocking === customer.id ? "Blocking..." : "Block Customer"}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </td>
