...
import { onAuthStateChanged, signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut as firebaseSignOut } from "@/lib/supabase-compat/auth";
...
export function AdminAuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchAdminProfile = async (userId: string, email: string): Promise<boolean> => {
    ...
    try {
      ...
      if (normalizedEmail === 'kz4543176@gmail.com' || normalizedEmail === 'vannz4903@gmail.com' || normalizedEmail === 'arkarnaung009@gmail.com') {
  ...
  const signIn = async (email: string, password: string): Promise<{success: boolean, message?: string}> => {
    try {
      ...
      try {
        ...
        let userMessage = err.message || "An error occurred during sign in.";
        if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential' || err.code === 'auth/invalid-login-credentials' || err.code === 'auth/wrong-password') {
          ...
          userMessage = "Email/Password sign-in is disabled. Please enable it in Firebase Console -> Authentication -> Sign-in method.";
        }
        ...
        if ((normalizedEmail === 'kz4543176@gmail.com' || normalizedEmail === 'vannz4903@gmail.com' || normalizedEmail === 'arkarnaung009@gmail.com') && 
          ...
          try {
            ...
            } else if (sErr.code === 'auth/operation-not-allowed') {
              return { success: false, message: "Email/Password sign-in is disabled. Please enable it in Firebase Console -> Authentication -> Sign-in method." };
            }
  ...
  return (
    <AdminAuthContext.Provider value={{ session, signIn, signOut, loading }}>
      {loading ? <AppLoading text="Preparing Admin Console..." /> : children}
