...
export function CustomerAuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchCustomerProfile = async (userId: string, email: string) => {
    try {
      ...
      if (!userDoc.exists()) {
        console.warn("Customer profile not found. Auto-creating customer record.");
        await setDoc(doc(db, 'users', userId), { role: 'customer', email, created_at: new Date().toISOString() });
  ...
  return (
    <CustomerAuthContext.Provider value={{ user, isAuthenticated: !!user, login, register, logout }}>
      {loading ? <AppLoading text="Loading your products..." /> : children}
