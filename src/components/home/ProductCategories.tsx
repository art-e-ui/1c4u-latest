...
export function ProductCategories() {
  ...
  const isWorkingImage = (url: string | undefined | null) => {
    if (!url) return false;
  ...
  const workingCategories = categories.filter(cat => isWorkingImage(cat.image));
                ...
                <div className="h-16 w-16 overflow-hidden rounded-full border-2 border-border bg-muted transition-all group-hover:border-primary group-hover:shadow-md md:h-20 md:w-20">
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      target.src = getCategoryImage(cat.slug);
                    }}
                  />
                </div>
