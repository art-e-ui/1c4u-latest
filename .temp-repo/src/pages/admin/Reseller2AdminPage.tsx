...
export default function Reseller2AdminPage() {
  ...
  }, [rawResellers, canSeeAll, allowedReferralIds, allowedStaffIds, allowedStaffDocIds]);
  ...
  const displaySessions = useMemo(() => {
    const sessionsFromResellers = allResellers.map(reseller => {
      ...
      return {
        ...
        reseller_name: reseller.shopName || reseller.name,
        reseller_avatar: "",
        is_online: false,
        is_pinned: false,
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      // Sort in memory to keep chronological order for display
